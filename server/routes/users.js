...
router.get("/api/:type/products", async (req, res) => {
  try {
    ...
    let ownerType;
    if (buyerType === "Customer") ownerType = "Retailer";
    else ownerType = "Wholesaler";

    let products = [];

    if (category) products = await Product.find({ ownerType, category });
    else products = await Product.find({ ownerType });
...
router.get("/api/:type/all", async (req, res) => {
  try {
    ...
    if (type === "Customer") users = await Customer.find({});
    if (type === "Retailer") users = await Retailer.find({});
    if (type === "Wholesaler") users = await Wholesaler.find({});
    if (type === "Delivery") users = await Delivery.find({});
