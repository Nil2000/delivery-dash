...
router.patch("/api/:type/buy/:id", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < cartItems.length; i++) {
      ...
      else productGroups[sellerId] = [copyProd];
      ...
      if (buyerProduct) buyerProduct["quantity"] += buyingQty;
      else {
      ...
      else transaction[sellerId] = buyingQty * price;
    }
...
router.patch("/api/:type/cart/:id", async (req, res) => {
  try {
    ...
    else return res.status(400).send({ error: "Cannot add more items" });
